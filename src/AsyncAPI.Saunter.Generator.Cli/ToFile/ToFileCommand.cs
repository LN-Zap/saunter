...
{
    internal class ToFileCommand(
        ILogger<ToFileCommand> logger,
        IEnvironmentBuilder environment,
        IServiceProviderBuilder builder,
        IAsyncApiDocumentExtractor docExtractor,
        IFileWriter fileWriter)
    {
        ...
        /// <param name="doc">
        ///     -d,name(s) of the AsyncAPI documents you want to export as configured in your startup class. To
        ///     export all documents using null.
        /// </param>
        ...
        /// <param name="env">
        ///     define environment variable(s) for the application. Formatted as a comma separated list of
        ///     _key=value_ pairs
        /// </param>
        [Command("tofile")]
        public int ToFile([Argument] string startupassembly, string output = "./", string doc = null,
            string format = "json", string filename = DEFAULT_FILENAME, string env = "")
        {
            ...
            var documents = docExtractor.GetAsyncApiDocument(serviceProvider,
            ...
            foreach (var (documentName, asyncApiDocument) in documents)
            {
                ...
                if (exportJson)
                {
                    fileWriter.Write(outputPath, fileTemplate, documentName, "json",
                        stream => asyncApiDocument.SerializeAsJson(stream, AsyncApiVersion.AsyncApi2_0));
                }

                if (exportYml)
                {
                    fileWriter.Write(outputPath, fileTemplate, documentName, "yml",
                        stream => asyncApiDocument.SerializeAsYaml(stream, AsyncApiVersion.AsyncApi2_0));
                }

                if (exportYaml)
                {
                    fileWriter.Write(outputPath, fileTemplate, documentName, "yaml",
                        stream => asyncApiDocument.SerializeAsYaml(stream, AsyncApiVersion.AsyncApi2_0));
                }
            }
