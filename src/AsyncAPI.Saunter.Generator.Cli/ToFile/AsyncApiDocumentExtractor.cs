...
namespace AsyncAPI.Saunter.Generator.Cli.ToFile
{
    ...
    {
        IEnumerable<(string name, AsyncApiDocument document)> GetAsyncApiDocument(IServiceProvider serviceProvider,
    ...
    internal class AsyncApiDocumentExtractor(ILogger<AsyncApiDocumentExtractor> logger) : IAsyncApiDocumentExtractor
    {
        private IEnumerable<string> GetDocumentNames(string[] requestedDocuments, AsyncApiOptions asyncApiOptions)
        {
            ...
            if (documentNames.Count == 0)
            {
                ...
                {
                    logger.LogCritical(
                        $"111. AsyncAPI documents found. Known named document(s): {string.Join(", ", asyncApiOptions.NamedApis.Keys)}.");
                    documentNames = [null]; // null marks the default, unnamed, document
                }
            }
        ...
        public IEnumerable<(string name, AsyncApiDocument document)> GetAsyncApiDocument(
            IServiceProvider serviceProvider, string[] requestedDocuments)
        {
            ...
            foreach (var documentName in GetDocumentNames(requestedDocuments, asyncApiOptions))
            {
                ...
                Console.WriteLine(asyncApiSchemaJson);
                AsyncApiDocument asyncApiDocument;
                try
                {
                    asyncApiDocument = new AsyncApiStringReader().Read(asyncApiSchemaJson, out var diagnostic);
                    if (diagnostic.Errors.Any())
                    {
                        logger.LogError(
                            $"AsyncAPI Schema '{documentName ?? "default"}' is not valid ({diagnostic.Errors.Count} Error(s))");
                    ...
                    {
                        logger.LogWarning(
                            $"AsyncAPI Schema '{documentName ?? "default"}' has {diagnostic.Warnings.Count} Warning(s):");
                ...
                {
                    logger.LogError(e, $"Failed to parse AsyncAPI document '{documentName ?? "default"}'");
                    throw;
                }
